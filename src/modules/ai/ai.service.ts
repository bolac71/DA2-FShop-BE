...
export class AiService {
  ...
  async virtualTryon(
    ...
    garmentDesc: string = 'A nice shirt',
    denoiseSteps: number = 30,
    seed: number = 42,
  ): Promise<Buffer> {
    try {
      const formData = new FormData();
      ...
      formData.append('denoise_steps', String(denoiseSteps));
      formData.append('seed', String(seed));

      this.logger.debug(`Sending virtual try-on request to AI service: ${this.aiServiceUrl}/tryon`);

      const response = await fetch(`${this.aiServiceUrl}/tryon`, {
        method: 'POST',
        body: formData,
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Virtual try-on failed: ${errorMessage}`);
